...
public class FishDAO {
	...
	public static void main(String[] args) {
		...
		while (!file.equals("")) {
			...
			if (Fish.isThisAFish(fishDescription)) {
				fish = Fish.getFishFromDescription(fishDescription);
				insertFish(fish);
			} else {
	...
	public static boolean insertFish(Fish fish) {
		...
		try {
			...
			stInsertFish.setString(4, fish.getWeathers().toString());
			...
			stInsertFish.setInt(8, fish.getPauseStart()); stInsertFish.setInt(9, fish.getPauseEnd());
	...
	public static List<Fish> getAllFishes() {
		...
		try {
			String reqSelectFish = "SELECT * FROM fish";
			PreparedStatement stSelectFish = DBConnection.getInstance().prepareStatement(reqSelectFish);
			ResultSet rsSelectFish = stSelectFish.executeQuery();
			Fish fish;
			while (rsSelectFish.next()) {
				Integer[] availabilities = new Integer[4];
				availabilities[0] = rsSelectFish.getInt(6); availabilities[1] = rsSelectFish.getInt(7);
				availabilities[2] = rsSelectFish.getInt(8); availabilities[3] = rsSelectFish.getInt(9);
				fish = new Fish(rsSelectFish.getInt(1),rsSelectFish.getString(2), 
						Season.getSeasonsFromString(rsSelectFish.getString(3)), 
						Weather.valueOf(rsSelectFish.getString(4)), null, 
						availabilities,rsSelectFish.getInt(10));
				fishes.add(fish);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
